...
from PySide6.QtWidgets import (QMainWindow, QApplication, QPushButton, QTreeWidget, QTreeWidgetItem,
                            ...
                            QMessageBox, QDialog, QStyledItemDelegate, QHeaderView, QListView)
...
from MainPage_V4_ui import Ui_MainWindow
...
from HorzintalBarPlot import BarGraphWidget
...
class SetupWindow(QMainWindow):
    ...
    def populate_table(self, SetupCase):
        ...
        # Create the delegate and apply it to the second column to force Hazard clases between 1 and 10
        if SetupCase == "HazardSetup":
    ...
    def add_row(self):  
        ...
        if self.SetupOption == "HazardSetup":
            record.setValue("HazardName", "NewHazard")
    ...
    def closeSetupWindow(self):
        if self.SetupOption == "HazardSetup":
            ...
            for row in range(self.model.rowCount()):
                ...
                if not value:
                    QMessageBox.critical(self, "Error", "Number of hazard classes mus be fillled (10 maximumm)!")
                    return
...
class MainWindow(QMainWindow):
    ...
    def onSecondaryWindowClosed(self, SetupTable):
        self.updatePerformanceTablesViews(self.models, self.table_views)
        updateHazardTableViews(self.ui.Performance_MainWidget)
    ...
    def onPSSComboBoxTextChanged(self):
        ...
        if self.baseline_scenario:                  
            self.SPR_plot.update_series_visibility(self.baseline_scenario, False)          #turn off the current baseline scenario
            
            self.additional_scenarios = getQListSelection(self.ScenarioList_model)          # Get the items that are selected on the QListView
            if self.additional_scenarios:                                                   #if other scenarios are activated
                for additional_scenario in self.additional_scenarios:
                    self.SPR_plot.update_series_visibility(additional_scenario, False)        #turn off those scenarios
        ...
        if new_baseline_scenario:                                                           #update the items to be shown on the QListView
            ...
            self.SPR_plot.update_series_visibility(new_baseline_scenario, True)             #activate the nre baseline scenario on the plot
            self.SPR_plot.set_baseline_scenario(new_baseline_scenario)                      #give to the plot class the name of the baseline scenario
            # self.SPR_plot.clear_hlines_and_texts()
    ...
    def updateScatterVisibility(self, index):
        ...
        self.SPR_plot.update_series_visibility(series_name, visibility)
        
        if visibility == False:
            self.SPR_plot.clear_hlines_and_texts(series_name)
    ...
    def plotFunctionals(self, DataFrame = pd.DataFrame, xScale = 100 or 1, dataColumn = str, yPrefix = str, DestinyWidget = QWidget):        
        ...
        Graph_plotter = BarGraphWidget(Categories, Values, xScale)  
    ...
    def plotPerformances(self, DataFrame = pd.DataFrame, xScale = 100 or 1 ,DestinyWidget = QWidget):
